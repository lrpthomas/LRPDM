...
class SpatialPerformanceAuditor {
  ...
  async runFullAudit(): Promise<void> {
    console.log('🔍 Starting comprehensive spatial performance audit...\n');
  ...
  private async checkPostGISConfiguration(): Promise<void> {
    console.log('📋 Checking PostGIS configuration...');
  ...
  private async analyzeSpatialIndexes(): Promise<void> {
    console.log('📊 Analyzing spatial indexes...');
    ...
    for (const col of geometryColumns.rows) {
      ...
      if (hasIndex.rows[0].count === 0) {
        this.results.recommendations.push(
          `❌ Missing GIST index on ${col.f_table_name}.${col.f_geometry_column}`
        );
      }
    }
  }
  ...
  private async analyzeTableStatistics(): Promise<void> {
    ...
    for (const table of stats.rows) {
      ...
      if (deadTupleRatio > 0.1) {
        this.results.recommendations.push(
          `⚠️ Table ${table.tablename} has ${(deadTupleRatio * 100).toFixed(1)}% dead tuples - consider VACUUM`
        );
      }
      ...
      if (daysSinceAnalyze > 7) {
        this.results.recommendations.push(
          `📊 Table ${table.tablename} last analyzed ${daysSinceAnalyze} days ago - consider ANALYZE`
        );
      }
    }
  }
  ...
  private async analyzeSpatialQueries(): Promise<void> {
    console.log('🔍 Analyzing spatial query patterns...');
  ...
  private async checkGeometryComplexity(): Promise<void> {
    console.log('🔢 Checking geometry complexity...');
    ...
    try {
      ...
      for (const stat of complexityStats.rows) {
        if (stat.max_points > 10000) {
          this.results.recommendations.push(
            `⚡ Feature type '${stat.feature_type}' has geometries with up to ${stat.max_points} points - consider simplification`
          );
        }

        if (stat.avg_size_bytes > 100000) {
          this.results.recommendations.push(
            `💾 Feature type '${stat.feature_type}' has large geometries (avg ${Math.round(stat.avg_size_bytes / 1024)}KB) - consider LOD strategy`
          );
        }
      }
  ...
  private async analyzeStorageUsage(): Promise<void> {
    ...
    for (const table of storage.rows) {
      if (table.index_ratio > 80) {
        this.results.recommendations.push(
          `📦 Table ${table.tablename} has ${table.index_ratio}% index overhead - review index strategy`
        );
      }
    }
  ...
  private async benchmarkCommonOperations(): Promise<void> {
    console.log('⏱️ Benchmarking common spatial operations...');
  ...
  private generateRecommendations(): void {
    ...
    if (criticalMetrics.length > 0) {
      this.results.recommendations.unshift(
        `🚨 ${criticalMetrics.length} critical performance issues found - immediate attention required`
      );
    }

    if (warningMetrics.length > 0) {
      this.results.recommendations.unshift(
        `⚠️ ${warningMetrics.length} performance warnings found - optimization recommended`
      );
    }
    ...
    this.results.recommendations.push(
      ...
      '  • Run VACUUM ANALYZE regularly on spatial tables',
      '  • Consider partitioning large spatial tables by geographic region',
      '  • Use appropriate SRID for your data (local projections for better performance)',
      '  • Implement geometry simplification for web map tiles',
  ...
  private printSummary(): void {
    console.log('\n' + '='.repeat(60));
    console.log('📊 SPATIAL PERFORMANCE AUDIT SUMMARY');
    ...
    this.results.indexes.slice(0, 5).forEach(idx => {
      console.log(`  📊 ${idx.table}.${idx.index} (${idx.type}): ${idx.scans} scans, ${idx.efficiency}% efficiency`);
    });

    console.log('\n⏱️ Query Performance:');
    this.results.queries.forEach(query => {
      console.log(`  🔍 ${query.query}: ${query.avgTime.toFixed(2)}ms avg`);
    });
