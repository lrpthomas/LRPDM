...
import { validateSupportedFileType } from '../utils/fileValidation';
...
export class CSVUploadController {
  static async uploadCSV(
    request: FastifyRequest<CSVUploadRequest> & FileUploadRequest,
    reply: FastifyReply
  ): Promise<void> {
    ...
    try {
      ...
      // Validate file type
      const fileType = validateSupportedFileType(request.file.filename, request.file.mimetype);
      if (fileType !== 'csv') {
        const error: APIError = {
      ...
      // Parse CSV data
      const csvData = await this.parseCSVStream(request.file.file);
      ...
      if (csvData.length === 0) {
        const error: APIError = {
          ...
          code: 'EMPTY_CSV_FILE',
          details: { filename: request.file.filename }
        };
        return reply.status(400).send({ error });
      }
      ...
      reply.status(500).send({ error: apiError });
    }
  }

  private static async parseCSVStream(stream: NodeJS.ReadableStream): Promise<Record<string, any>[]> {
    ...
    try {
      await pipeline(
        ...
        parse({
          ...
          trim: true,
          max_limit_on_data_read: 50 * 1024 * 1024, // 50MB limit
  ...
  static async previewCSV(
    request: FastifyRequest & FileUploadRequest,
    reply: FastifyReply
  ): Promise<void> {
    try {
      if (!request.file) {
      ...
      const previewData = await this.parseCSVPreview(request.file.file, 10);
  ...
  private static async parseCSVPreview(stream: NodeJS.ReadableStream, limit: number): Promise<Record<string, any>[]> {
    const records: Record<string, any>[] = [];
    let count = 0;
